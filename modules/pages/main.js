...
router.get('/nextAds', require('../role/any'), async (req,res,next)=>{
    ...
    if(req.session.leftVideos){
        ...
        if(results.length < 1){
            results = await (await fetch('https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&key=AIzaSyAB7jesp2Et9CF32yNtPuNdB2LySYGmoL4&playlistId=PLvagogV4avthbgsEv70XpQp_Y9i5XEAgU&maxResults=50', { method:'get' })).json();
    ...
    function replysfn(inner_replys, lpad){
        ...
        for(let reply of inner_replys){
            ...
            += `<details style="padding-left:${lpad}px">
                    ...
                    <form action="${`/board/${req.params.id}`}${`?page=${page}${keyword ? `&keyword=${keyword}` : ""}`}" method="post">
